...
public class PushMessageReceiver extends BroadcastReceiver {
	...
	//final String urlString="http://192.168.17.44:8004/httpserver/httpserver.action";
	final String urlString="http://192.168.3.175/MServer/updateEmpChannelInfo.search";
	...
	public void onReceive(final Context context, Intent intent) {
		...
		if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
			...
			final String paraString = String.format("action=getTagInfo&appid=%s",APP_ID);
			//String recvMessageString=sendPost(urlString, paraString);
			new Thread(){
				@Override
				public void run(){
				//你要执行的方法
					String recvMessageString=HttpRequest.sendPost(urlString,  paraString);
			...
			if (method.equalsIgnoreCase(METHOD_SET_TAG)) {
				if (errorCode==0) {
					...
					try {
						responseObject = initJsonObject.getJSONObject("response_params");
					...
					//向服务器发送绑定参数信息					
					final String paraString = String.format("action=setTag&user_id=%s&tag=%s",
							USER_ID	,paramPost);
					...
					new Thread(){
						...
						//你要执行的方法
							...
							if (recvMessageString=="") {
								//返回给JS调用
								PushDemoActivity.cbContext.error(ErrorPush.NET_ERROR.toString());
							}	
						}
					}.start();
			...
			}else if(method.equalsIgnoreCase(METHOD_DEL_TAG))
			{
				if (errorCode==0) {
					...
					//向服务器发送绑定参数信息
					//final String urlString="http://192.168.17.44:8080/httpserver/httpserver.action";			
					final String paraString= String.format("action=delTag&user_id=%s&tag=%s",
							USER_ID	,paramPost);
					//String recvMessageString=sendPost(urlString, paraString);	
					new Thread(){
						@Override
				...
				if(errorCode==0)
				{
					...
					try {
						paramJsonObject=getJSONObject(initJsonObject.getString("response_params"));
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}					
					...
					try {
						final String paraString = String.format("user_name=%s&action=initUser&appid=%s&user_id=%s&channel_id=%s&platform=android", 
								UNAME,paramJsonObject.getString("appid"),paramJsonObject.getString("user_id"),paramJsonObject.getString("channel_id"));
						Log.d(TAG, "paramString : " + paraString);
						new Thread(){
							@Override
							public void run(){
							//你要执行的方法
								String ssString=HttpRequest.sendPost(urlString,  paraString);
				...
				if(errorCode==0)
				{
					...
					//返回给JS调用的tags信息			
					String listTags=null;
					try {
						listTags=paramJsonObject.getString("groups");
					} catch (JSONException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
						PushDemoActivity.cbContext.error(ErrorPush.MESSAHE_ERROR.toString());
